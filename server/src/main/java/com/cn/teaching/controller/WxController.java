...
public class WxController {
    ...
    @Autowired
    private TtSiteAppointmentService ttSiteAppointmentService;
    ...
    public R getSiteById(String id, String userId){
        TtSite ttSite = ttSiteService.getById(id);
        // 查询当前用是否已经预约过了
        ttSite.setFlag(ttSiteAppointmentService.getBySiteIdAndUserId(id, userId));
        return R.ok().data("row", ttSite);
    }

    /**
     * 预约场地
     */
    @PostMapping("appointmentSite")
    public R appointmentSite(@RequestBody TtSiteAppointment ttSiteAppointment){
        // 判断当前用户是否已经预约过了
        boolean flag = ttSiteAppointmentService.getBySiteIdAndUserId(ttSiteAppointment.getId(), ttSiteAppointment.getUserId());
        if(flag){
            return R.error("您已经预约过了");
        }
        ttSiteAppointment.setStatus("1");
        ttSiteAppointmentService.save(ttSiteAppointment);
        return R.ok();
    }
     ...
     * 取消场地预约
     */
    @PostMapping("cancelAppointmentSite")
    public R cancelAppointmentSite(@RequestBody TtSiteAppointment ttSiteAppointment){
        ttSiteAppointmentService.cancelAppointmentSite(ttSiteAppointment);
        return R.ok();
    }
     ...
     * @return
     */
    @GetMapping("getSiteAppointmentInfo")
    public R getSiteAppointmentInfo(String id,String status){
        List<TtSiteAppointment> list =  ttSiteAppointmentService.getSiteAppointmentInfo(id, status);
        return R.ok().data("row", list);
    }
    ...
    public R sendLeave(@RequestBody TtLeave ttLeave){
        ttLeave.setStatus("0");
        ttLeaveService.save(ttLeave);
        return R.ok();
    }
     ...
     * 查询请销假记录
     * @param id 用户id
     * @param type 类型 (0请假 1销假)
     * @return
     */
    @GetMapping("getLeaveAppointmentInfo")
    public R getLeave(String id,String type){
        List<TtLeave> list = ttLeaveService.getLeaveByLeaveIdAndType(id,type);
        return R.ok().data("row", list);
    }
    ...
    @GetMapping("getSite")
    public R getSite(){
        List<TtSite>courseList = ttSiteService.list(new QueryWrapper<TtSite>().eq("status", "0"));
        return R.ok().data("row", courseList);
    }
